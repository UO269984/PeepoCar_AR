    ...
    SubShader {
        ...
        Pass {
            ...
            half4 frag(v2f i) : COLOR 
            {
                ...
                float x1_depth = 0.0;
                DecodeDepthNormal(tex2D(_CameraDepthNormalsTexture, uv + float2(-du, 0.0)), x1_depth, x1_normal);

                half3 x2_normal = half3(0.0,0.0,1.0);
                float x2_depth = 0.0;
                DecodeDepthNormal(tex2D(_CameraDepthNormalsTexture, uv + float2(du, 0.0)), x2_depth, x2_normal);

                half3 y1_normal = half3(0.0,0.0,1.0);
                float y1_depth = 0.0;
                DecodeDepthNormal(tex2D(_CameraDepthNormalsTexture, uv + float2(0.0, -dv)), y1_depth, y1_normal);

                half3 y2_normal = half3(0.0,0.0,1.0);
                float y2_depth = 0.0;
                DecodeDepthNormal(tex2D(_CameraDepthNormalsTexture, uv + float2(0.0, dv)), y2_depth, y2_normal);
